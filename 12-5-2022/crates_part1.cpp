...
/**
 * @brief Moves crates based on input specifications. 
 * 
...
void move_crates(vector<stack<char>>& stacks, size_t num, size_t source, size_t dest) {
    for (size_t i = 0; i < num; ++i) {
        char to_move = stacks[source].top();
        stacks[source].pop();
...
int main(int argc, char** argv) {
    ...
    while(iss >> current_stack) {
        ...
        // Line below seperator is crate numbers, line before that is the actual crates
    ...
    for (size_t i = seperator_line + 1; i < lines.size(); ++i) {
        ...
        // 2nd word is the num of crates to move, 4th is the source, and 6th is the destination
